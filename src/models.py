...
from .utils import AUTO, get_chained_attr, logged
from .base import Model
from .data import AnnDataset, ArrayDataset, DataLoader, GraphDataset
from .nn import freeze_running_stats

from .modules import GLUE, GLUETrainer
from . import sc

from .loss import SelfEntropyLoss, DDCLoss
...
@logged
class CoVELTrainer(GLUETrainer):

    BURNIN_NOISE_EXAG: float = 1.5  # Burn-in noise exaggeration

    def __init__(
            self, net: CoVEL, lam_data: float = None, lam_kl: float = None,
        ...
        if net.u2c:
            ...
            # print(net.u2c.out_features)
            self.ddc = DDCLoss(net.u2c.out_features, loss_weights)
    ...
    def compute_losses(
        ...
        if net.u2c:
            ...
            lmsk = xlbl_cat >= 0
            # print("u_cat",u_cat[lmsk].size())
            # print("xlbl",xlbl_cat[lmsk].size())
...
@logged
class CoVELModel(Model):

    NET_TYPE = CoVEL
    TRAINER_TYPE = CoVELTrainer
    ...
    def adopt_pretrained_model(
            self, source: "CoVELModel", submodule: Optional[str] = None
    ) -> None:
        r"""
    ...
    def __repr__(self) -> str:
        return (
            f"CoVEL model with the following network and trainer:\n\n"
            f"{repr(self.net)}\n\n"
            f"{repr(self.trainer)}\n"
        )