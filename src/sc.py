...
class GraphEncoder(modules.GraphEncoder):
...
class GraphDecoder(modules.GraphDecoder):
...
class DataEncoder(modules.DataEncoder):
...
class DataDecoder(modules.DataDecoder):
...
class Discriminator(torch.nn.Sequential, modules.Discriminator):
